...
function solveLinkage(a, b, c, d, theta1Deg) {
    ...
    // Two possible solutions, choose "elbow up"
    const C = {
        x: B.x + x*ex - y*ey,
        y: B.y + x*ey + y*ex
    };
...
function drawLinkage(a, b, c, d, theta1Deg, e = 60, alpha = 30) {
    ...
    for (let theta = 0; theta <= 360; theta += 2) {
        let res = solveLinkage(a, b, c, d, theta);
        if (res) {
    ...
    const result = solveLinkage(a, b, c, d, theta1Deg);
    if (!result) {
        ctx.save();
        ctx.fillStyle = 'red';
        ctx.font = '20px Arial';
...
function plotAngles(a, b, c, d, currentInputAngle = null) {
    ...
    let inputAngles = [];
    for (let theta = 0; theta <= 360; theta += 2) {
        let result = solveLinkage(a, b, c, d, theta);
        if (result) {
    ...
    if (currentInputAngle !== null) {
        ...
        let result = solveLinkage(a, b, c, d, currentInputAngle);
        if (result) {
